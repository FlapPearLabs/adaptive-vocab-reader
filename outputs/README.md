...
12. [V0.1 Dogfood 交互个性化回退与测评重对齐规格（待审）](../docs/specs/2026-08-11-V0.1-Dogfood-交互个性化回退与测评重对齐规格.md)：2026-08-11 唯一 Spec（DRAFT/待审）；由已通过 DOCUMENT 复审 PASS 的 Grill 决议 D1–D17（`work/2026-08-11-v0.1-dogfood-realignment-grill-decisions.md`）转换而来。取代 2026-08-06 Spec 中与 D1–D17 冲突的合同（交互 Ctrl 门控/正文 click 归网页、Evidence 参与 hint、未收录可反馈、用户主动单词级网络回退、重测半重置等），未取代合同继续继承。**批准 ≠ 实现**；不授权开发，拆 ticket 与开始开发须用户另行明确授权。
