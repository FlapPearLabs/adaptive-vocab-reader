 ...
 * 唯一可见后果是省略行内释义并在 tooltip / 检查浮层内给出本固定文案，不合成占位释义。
 * 集成整合：T-VUX-1（轻提示路径）与 T-VUX-2（检查浮层路径）曾各自定义同值常量，
 * 集成 lane 按批次 README §6 合并为本单一权威常量。
 */
...
function metadataOrFallback(value: string | undefined): string {
  ...
  return trimmed.length > 0 ? trimmed : METADATA_RESOLUTION_FAILURE_TEXT;
}
