...
5. `outputs/v0.1-产品规格与开发计划.md` 是已废弃的较早草案，不得作为实现依据；当前产品规格、开发计划和验收计划以 `outputs/README.md` 登记的文档为准。
...
- 对 lemma、词形、粗义项、MWE、派生和中文释义映射建立人工金标 fixture；
- 重点覆盖 `run`、`running`、`runner`、`rerun`、`run into` 和不同义项，防止词族状态错误传播；
- 数据更新必须有 schema/version 迁移、前后 diff 和回滚方案。

### 6.3 个性化模型与状态

- 用户直接反馈、测评、复测、推断和时间衰减必须能被分别审计；
- 必须验证普通曝光不会直接变为掌握；
- 必须验证“我会了”立即撤除当前提示，“不会”立即产生当前语境提示；
- 必须验证不同义项和 MWE 不被基础 lemma 错误隐藏；
- 不能只以“提示变少”验收；至少同时衡量漏标、误提示和错误释义。具体阈值由 `RULES.md`/Spec 确认后补充。
...
- 验证选择单词和词组、撤销、刷新后持久化及网站 CSS 隔离；
- 记录长文扫描耗时、单批主线程耗时、DOM 增量和布局影响。

### 6.5 本地 Provider 与失败降级

- Ollama 和 LM Studio 使用真实 localhost 协议或忠实的本地协议 fixture 验证；
- 覆盖未启动、模型不存在、超时、取消、无效结构化输出和中途断连；
- 本地模型不可用时不得破坏本地词典和网页阅读主路径；
- 网络和权限测试必须证明实际行为与当时已确认的隐私边界一致。

### 6.6 存储、迁移和缓存

- 只有在 `RULES.md` 确认数据字段、容量、淘汰和压缩策略后才能冻结 schema；
- 验证升级、回滚、异常中断、数据损坏、容量到达上限和清空流程；
- 可重新生成的缓存和用户直接证据必须分层；直接证据能否压缩或删除，以 `RULES.md` 中后续确认的保留策略为准；
- 任何可能保存 URL、句子或上下文的变更都必须经过隐私边界审查。
