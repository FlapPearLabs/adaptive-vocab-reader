...
- **[已确认·已实现@2026-09-09]** 系统判断用户应该会的词：默认不显示灰线，但仍然能够查询，仍然能够反馈“不会”；用户反馈不会后进入生词本并显示红色提示。
- **[已确认·已实现@2026-09-09]** 用户明确反馈“不会”的词：显示红色强提示；同一 canonical wordKey 的相关页面实例同步；进入生词本。
- **[已确认·已实现@2026-09-09]** 用户明确反馈“会”或在生词本点击“已掌握”后：红色提示消失；生词本条目消失；但该词仍保持可查询、可再次纠错；known 不等于永远不可交互。
