...
- **[已确认]** 内容脚本只根据 `wordKey` 对应的 `WordState` 决定提示，不读取 `AssessmentEvidence`。新目标下展示决策拆为两个来源：显式状态提示（known 不提示 / learning 强提示）＋候选提示（仅对未显式反馈词是否显示灰线）。**OPEN_DECISIONS C 已决议验收方式并完成输入/算法对齐（2026-08-07）**：验收＝dogfood 记录每百词灰线密度 + 复用「不必要提示」人工验收，数据积累后再定阈值；**候选输入＝仅查询词典频率（frq/bnc）+ 用户显式状态过滤（排除 known/learning），不含 Evidence/band；算法＝频率下界阈值；首轮阈值由确定性 bootstrap 规则生成（不猜数字），随后由 dogfood 数据校准**。缺有效频率排名的可查询词不参与候选判定，但仍保持可查询。实现须待 Spec 批准。
...
- **[已确认·尚未实现]** 灰色虚线只用于系统判断为“用户潜在可能不会”的少量候选词。**OPEN_DECISIONS C 已决议验收方式并完成输入/算法对齐（2026-08-07）**：不预设固定密度数字，dogfood 期间记录每百词灰线密度作为参考数据，并复用现有「不必要提示」人工数字验收误提示，数据积累后再定阈值；**候选输入＝仅查询词典频率（frq/bnc）+ 用户显式状态过滤（排除 known/learning），不含 Evidence/band；算法＝频率下界阈值（首轮阈值由确定性 bootstrap 规则生成，不猜数字；随后用 dogfood 密度数据校准规则参数）；候选不落盘、不改写用户状态**。缺有效频率排名的可查询词不参与候选判定，但仍可查询、可反馈（query eligibility 与 hint eligibility 独立）。实现须待 Spec 批准；不得恢复高置信自动隐藏、Pool B、概率画像等冻结项（见「明确不做与冻结项」）。
...
- **[已确认]** 本轮采用**显式双门禁**（Spec §18）：**第一道门已通过（2026-08-07）**＝DOCUMENT 审查通过且用户接受（只确认高层方向与 A–F 决议记录，不授权生产）；**发现与对齐已完成**＝E 许可证/数据可行性调查（E_VALIDATED 限定本地范围）、D 可丢弃交互原型（推荐透明 span 包装+事件委托）、C 候选输入与算法对齐（仅频率+状态过滤、频率下界阈值，**首轮阈值由确定性 bootstrap 规则生成、dogfood 校准**），结果已回写 RULES 与唯一 Spec；**第二道门（当前）**＝回写后再走一次 DOCUMENT 审查与用户批准；**只有 D 最终路线与 C 候选合同明确（经批准）后**才允许 `/to-tickets` 拆生产垂直切片并交给 Codex。可丢弃原型不得演变为生产实现；核心算法与交互路线的最终选择**不得**延后到生产实施阶段才决定。E 验证范围仅限个人本地 dogfood、不公开发布；若需公开再分发，立即 STOP，E/F 返回用户重新决策（Spec §14 E、§16 fail-closed）。**数据合规**：原型/研究证据不得包含 ECDICT 派生音标/词性/中文释义 payload（仅结构化数据），违反即视为公开再分发风险（Spec §18 末段）。
