...
本文件是当前唯一的产品规则来源。用户最新明确指示优先；研究笔记只提供事实或建议，不自动成为需求。2026-07-30/31 Grill 阶段用户逐题确认的决定（GR-01～GR-11、CR-01～CR-06，完整追踪见 `docs/specs/2026-07-30-V0.1-重新对齐规格.md`）固化为现行规则，并删除或修正与其冲突的旧表述。2026-08-04 依据用户真实 Chrome 试用反馈（dist 构建、694 个命中词中 693 个为 unknown、learning 仅 1 个，轻提示悬停只有释义）新增三条阅读体验规则：轻提示 tooltip 补音标词性、拖选选区加词入口、popup 生词本页签。
