...
- **[已确认]** 本轮采用**显式双门禁**（Spec §18）：**第一道门已通过（2026-08-07）**＝DOCUMENT 审查通过且用户接受（只确认高层方向与 A–F 决议记录，不授权生产）；**发现与对齐已完成**＝E 许可证/数据可行性调查（E_VALIDATED 限定本地范围）、D 可丢弃交互原型（推荐透明 span 包装+事件委托）、C 候选输入与算法对齐（仅频率+状态过滤、频率下界阈值），结果已回写 RULES 与唯一 Spec；**第二道门（当前）**＝回写后再走一次 DOCUMENT 审查与用户批准；**只有 D 最终路线与 C 候选合同明确（经批准）后**才允许 `/to-tickets` 拆生产垂直切片并交给 Codex。可丢弃原型不得演变为生产实现；核心算法与交互路线的最终选择**不得**延后到生产实施阶段才决定。E 验证范围仅限个人本地 dogfood、不公开发布；若需公开再分发，立即 STOP，E/F 返回用户重新决策（Spec §14 E、§16 fail-closed）。
