...
- **[已确认]** 未获用户明确授权，不得实现生产功能、commit、push、创建 Issue/PR、部署、发布、下载词典或修改远端配置。用户已确认的常驻 Git 同步、网页版 GPT 审查、交接、行动提示词的当前任务/新任务路由与合并授权判据以 `AGENTS.md` 第 5 节为准；网页版 GPT 审查回复必须包含目标代理、当前任务/新任务路由和可转发的下一步提示词。本地 ticket 默认不创建 GitHub Issue。
