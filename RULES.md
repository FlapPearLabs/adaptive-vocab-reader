...
- **[已确认]** 当前已有可构建、可用真实 Chrome 验证的 1,000 词本地垂直切片。**2026-09-09 状态重标**：原句「词汇量估计、每日校准轮、wordKey 身份迁移……尚未完成」中，估计 / 每日校准轮 / wordKey 身份迁移（schema 3）三项**已完成并经 fresh 验证**；**仅「真人 dogfood 闭环」仍未完成**，因此 V0.1 仍不可称已产品验收。
...
- **[已确认·已实现@2026-09-09]** 查询词典与固定测评词包职责分离（见「查询、交互、主动提示与测评词包解耦」）：查询词典负责网页查词、词形解析、释义、会/不会反馈和生词本；固定测评词包负责首测、每日校准、frequency band、AssessmentEvidence 和词汇量估计。**OPEN_DECISIONS A/E 已决议；E 已验证通过（2026-08-07，限定个人本地 dogfood、不公开发布）**：查询词典来源方向为 ECDICT 全量本地词典（公开再分发权利链仍 UNKNOWN，若需公开则 fail-closed 返回用户重新决策，见 Spec §14 E、§18），具体条目数由数据可行性决定、不预设；未收录词给明确「未收录」响应；未达到实施状态前不得伪装为已批准或已实现。
...
- **[已确认·已实现@2026-09-09]** known 只代表用户当前明确认为自己会：不显示学习提示，但仍可查询，仍可重新反馈“不会”；known ≠ 永远不可交互。**2026-09-09 状态重标**：原句「当前实现中 known 词被还原为纯文本且无交互载体，属于实现差距」已被 fresh E2E 推翻——known 词现为**可查询透明 span**，刷新后仍保留交互载体（`e2e-verify.cjs` 断言 `.avr-word[data-word]`，className 仅剩 `avr-word`）。原句保留仅为存史，**不得再作为待实现工作**。
- **[已确认·已实现@2026-09-09]** 查询资格不得依赖 light/strong 包装：是否能够查询和反馈，不得由是否显示灰线决定；无灰线词仍可悬停查询、点击反馈“不会”。
- **[已确认·已实现@2026-09-09]** 用户显式状态（known / learning / 未显式反馈）与主动提示候选（是否显示灰线）是两个独立维度：未显式反馈 ≠ “不会”，系统预测“可能不会”也不等于用户已明确标记 learning；候选判断不得无条件改写用户明确状态（见「查询、交互、主动提示与测评词包解耦」）。

## 查询、交互、主动提示与测评词包解耦（2026-08-06 最新产品目标）

本小节是 2026-08-06 用户最新产品目标的规范落点，取代与本轮目标冲突的旧口径。**本节所列高层产品方向已由用户确认，且已于 2026-09-09 经 RESUME-01 fresh 验证为实现**（2026-08-06 原文为「已由用户确认，但尚未实现（当前 main 生产代码忠实实现了旧 RULES/Spec）」，该状态叙述已被 fresh E2E 推翻，保留仅为存史；GOAL 1–7 逐条对照见 [`docs/CURRENT_IMPLEMENTATION_BASELINE.md`](docs/CURRENT_IMPLEMENTATION_BASELINE.md) §2–§3 与 UX 集成规格 §5）；本节不授权任何开发。**OPEN_DECISIONS A–F 已于 2026-08-06 由用户逐项决议，并于 2026-08-07 完成发现与对齐，唯一 Spec 已于 2026-08-07 经第二道 DOCUMENT 复审 `PASS` 与用户最终批准（APPROVED）**（覆盖验收=dogfood 驱动；未收录=仅提示不进生词本；词典来源=ECDICT 全量本地（**E 已验证，限定个人本地 dogfood、不公开发布**）；包外身份=同键不区分来源不升 schema（**F 已随 E 生效**）；交互路线=可丢弃原型已完成并**已批准生产实施路线＝透明 span 包装 + 事件委托，caret 不作唯一交互基础**；稀疏验收=dogfood 密度+不必要提示人工验收，**候选输入/算法已对齐：仅频率 frq/bnc+显式状态过滤（不含 Evidence/band）、频率下界阈值，数字由 dogfood 校准**），具体决议与状态见 Spec §14。**批准 ≠ 已实现**（**2026-09-09 重标：本节的解耦目标现已实现；Spec §16 差距清单已过期，剩余差距见 `docs/CURRENT_IMPLEMENTATION_BASELINE.md` §3 与 UX 集成规格 §7**），实现仍须经后续 `/to-tickets` 阶段与用户“开始开发”授权。
...
- **[已确认·已实现@2026-09-09]** 普通英文网页中的普通英文词，原则上都应能够：查询词义；看到原词或词形、音标、词性和中文释义；反馈“会”或“不会”；标记“不会”后进入生词本；标记“会”后从生词本移除。**OPEN_DECISIONS A 已决议**：不设语料阈值，覆盖验收采用 dogfood 驱动——词典命中即交互、未命中给明确「未收录」响应（B），覆盖质量用 dogfood「覆盖缺失」人工数字驱动后续扩容决策；不预设条目数。
- **[已确认·已实现@2026-09-09]** 是否能够查询和反馈，不得由是否显示灰线决定；不允许因为一个词没有灰线，就完全无法悬停查询或反馈“不会”。
- **[已确认·已实现@2026-09-09]** 灰色虚线只用于系统判断为“用户潜在可能不会”的少量候选词。**OPEN_DECISIONS C 已决议验收方式并完成输入/算法/bootstrap 对齐（2026-08-07）**：不预设固定密度数字，dogfood 期间记录每百词灰线密度作为参考数据，并复用现有「不必要提示」人工数字验收误提示，数据积累后再定阈值；**候选输入＝仅查询词典频率（frq/bnc）+ 用户显式状态过滤（排除 known/learning），不含 Evidence/band；算法＝频率下界阈值；首轮 bootstrap 公式（用户确认）＝有效频率 rank 升序列表中间索引阈值 S[⌊n/2⌋]（0-based；偶数 n 时取两个中间元素中的上侧元素）**；light 候选判定方向与边界（用户确认）：`effectiveFrequencyRank > 阈值`（严格大于，等于不提示）；frq/bnc 均无效时 hint-ineligible 但仍 queryable；随后用 dogfood 密度数据校准分位点；候选不落盘、不改写用户状态。缺有效频率排名的可查询词不参与候选判定，但仍可查询、可反馈（query eligibility 与 hint eligibility 独立）。实现须经后续 `/to-tickets` 阶段与用户“开始开发”授权；不得恢复高置信自动隐藏、Pool B、概率画像等冻结项（见「明确不做与冻结项」）。
- **[已确认·已实现@2026-09-09·配色已由 DEC-1 重标]** 系统判断用户应该会的词：默认不显示灰线，但仍然能够查询，仍然能够反馈“不会”；用户反馈不会后进入生词本并显示**琥珀系强提示**。（**DEC-1 重标**：原「红色提示」配色于 2026-09-10 由用户裁决改为琥珀系，见「V0.1 呈现层与流程裁决」DEC-1；本条的**行为部分已实现且不受影响**，仅配色改变。）
- **[已确认·已实现@2026-09-09·配色已由 DEC-1 重标]** 用户明确反馈“不会”的词：显示**琥珀系实线强提示**；同一 canonical wordKey 的相关页面实例同步；进入生词本。（**DEC-1 重标**：原「红色强提示」配色表述保留仅为存史，**不得再作为当前配色规范**；learning 与 light 的**强度区分**（实线 vs 点线）保持不变。）
- **[已确认·已实现@2026-09-09·配色已由 DEC-1 重标]** 用户明确反馈“会”或在生词本点击“已掌握”后：琥珀强提示消失；生词本条目消失；但该词仍保持可查询、可再次纠错；known 不等于永远不可交互。
- **[已确认·已实现@2026-09-09]** 查询词典与固定测评词包职责分离：查询词典负责网页查词、词形解析、释义、会/不会反馈和生词本；固定测评词包负责首测、每日校准、frequency band、AssessmentEvidence 和词汇量估计。查询词典内、测评包外的词：原则上允许持有用户状态、允许进入生词本；不进入 AssessmentEvidence；不参与词汇量估计。**OPEN_DECISIONS E/F 已决议并生效（2026-08-07，限定范围）**：查询词典方向为 ECDICT 全量本地——**E 已验证通过**（MIT 正面授权个人本地使用与打包，限定个人本地 dogfood、不公开发布；公开再分发权利链仍 UNKNOWN，若需公开则 E/F 返回用户重新决策，fail-closed）；**F 已随 E 生效**：包外词沿用与包内相同的字符串身份键、不区分来源、不升级 schema，隔离仅由约束层保证。
- **[已确认·已实现@2026-09-09]** 主动提示候选与用户状态分离：用户没有明确反馈 ≠ “不会”；系统预测“可能不会”也不等于用户已明确标记 learning；候选判断不得无条件改写用户明确状态；不得把系统预测静默写成用户明确状态。
- **[已确认·已实现@2026-09-09] frequency runtime 数据合同（查询词典的 hint 输入）**：新查询词典运行时须为 Hint selection 提供最小只读频率元数据 `effectiveFrequencyRank`（语义稳定；名称可调整，不得改变语义）。组合/回退规则沿用已确认口径：**`frq` 有效排名优先，`frq` 缺失时用 `bnc` 有效排名**（均无效时该词 hint-ineligible）。比较方向：**`effectiveFrequencyRank > 当前阈值` 才可能成为 light 候选**（rank 越大＝越生僻）。`effectiveFrequencyRank` **不是 AssessmentEvidence、不是 frequency band**，不进入估计、不改变 WordState；本条目只定义数据合同，不指定 JSON sidecar、字段布局、数据库或新 schema（见 Spec §5/§8/§9）。
- **[已确认·已实现@2026-09-09]** 核心交互路径：悬停可查询词 → 查看释义；点击可查询词 → 反馈会/不会。拖选加词保留为辅助入口，但不得成为无灰线词唯一的查询或纠错路径。
- **[已确认·已实现@2026-09-09]** tooltip 尽量不遮挡目标词和正文：优先显示在目标词上方，上方空间不足时显示在下方；与目标词及正文保留安全间距；不超出左右视口；不侵入可识别的 sticky/header 安全区域；页面滚动后定位仍正确。
- **[已确认·已实现@2026-09-09]** 真实鼠标操作必须成为验收路径，不能再只使用合成 `Range + mouseup` 代替真人拖选。
...
- **[已确认·已实现@2026-09-09]** 拖选加词保留为辅助入口，不得成为无灰线词唯一的查询或纠错路径；核心交互路径是「悬停可查询词 → 查看释义；点击可查询词 → 反馈会/不会」。
- **[已确认·已实现@2026-09-09]** tooltip 定位验收：tooltip 应尽量不遮挡目标词和正文——优先显示在目标词上方，上方空间不足时显示在下方；与目标词及正文保留安全间距；不超出左右视口；不侵入可识别的 sticky/header 安全区域；页面滚动后定位仍正确。**2026-09-09 状态重标**：原句「当前实现以目标词左上角为锚点、`top=y-8` 且只处理 bottom/right 溢出……属于实现差距」已被 fresh E2E 推翻——现已实现上方优先 / 空间不足下翻 / 不侵入 sticky header / 不越视口 / 滚动后定位正确。原句保留仅为存史，**不得再作为待实现工作**。
- **[已确认·已实现@2026-09-09]** 真实鼠标用户路径验收：真实 mouse down/move/up 拖选路径必须可验收；**2026-09-09 状态重标**：原句「现有 E2E 只用合成 `Range + mouseup`……属于测试 seam 差距」已被 fresh E2E 推翻——真实 `mousedown/mousemove/mouseup` 时间线与拖选后 click 序列已覆盖（`pageScanner.ts` + `e2e-verify.cjs` AC-9）。原句保留仅为存史，**不得再作为待实现工作**。
...
## V0.1 呈现层与流程裁决（2026-09-10 用户裁决，DEC-1~DEC-5）

本节是 `PHASE-DEC-TICKETS` 阶段由产品负责人逐项裁决的结果，详细对账与依据见 [`docs/specs/2026-09-10-V0.1-UX-V1.2.1-集成规格.md`](docs/specs/2026-09-10-V0.1-UX-V1.2.1-集成规格.md) §5/§7/§10。本节只规定**呈现层视觉、UI 文案语言、范围取舍与流程顺序**，**不改变** `WordState` / `wordKey` / `DictEntry` / `QuizQuestion` / `AssessmentEvidence` / 隐私与持久化契约。本节同样**不授权**开发、commit、push、合并或发布；拆票与开发仍须用户另行明确授权。

- **[已确认·DEC-1 = `AMBER_SYSTEM`]** V0.1 阅读面使用克制的**暖琥珀视觉族**：轻提示（light）＝淡琥珀**点线**下划线；强提示（learning）＝淡琥珀**实线**下划线；learning 同页首次出现额外显示克制的行内中文释义。**不得仅因 learning 状态而使用红色或刺眼警示色。** 冻结 UX 文档中的 `#f59e0b` / `#d97706` 及其透明度为**非规范性视觉参考**；除非实现确实需要引入设计 token，否则不得把具体 hex 值写成领域合同。light / learning 的**强度区分（点线 vs 实线）保持不变**，改变的只是色相。
- **[已确认·DEC-2 = `DEFER_SETTINGS_FROM_V0_1`]** V0.1 **不实现、不渲染 Settings 页签**，也不建设空壳页签、禁用占位页签或推测性设置控件——当前被列出的设置项全部为 OPEN / NOT AUTHORIZED 或明确禁止（见集成规格 U-39）。任何 Settings 相关工作须另行取得产品授权。
- **[已确认·DEC-3 = `CHINESE_FIRST`]** V0.1 面向用户的 UI 文案使用**简洁中文术语**（如 `会` / `不会` / `生词本` / `水平测评` / `不确定` / `释义暂不可用` / `当前词典未收录`）。**不得使用 `Mark as Learning · 不会` 这类中英混排标签**，除非存在真实的技术或语言学理由（该理由必须在规格中显式写明）。内部领域标识符（`known` / `learning` / `unknown` / `QuizQuestion` / `wordKey` 等）保持不变；**不得为翻译 UI 文案而重命名代码或领域契约**。
- **[已确认·DEC-4 = `DEFER_THIS_PAGE_FROM_V0_1`]** 新增的 popup「本页 / This Page」页面词表能力**不属于本轮 V0.1 实施波次**，记为未来候选，不进本批实施 ticket。（理由：它是 D-1~D-10 中唯一真正的新增产品能力，且完成冻结 UX 的阅读 / 查询 / 生词本 / 测评交互对齐并不需要它。）**本条只延后 popup 的「本页」产品面，不移除阅读面已需要的当前页瞬时处理。**
- **[已确认·DEC-5 = `IMPLEMENT_UX_DELTA_THEN_MANUAL_DOGFOOD`]** V0.1 收尾顺序固定为：已验证自动化基线（**RESUME-01 已建立**）→ 实施已批准的 V0.1 UX delta → 重跑 fresh 自动化门禁 → **真人 dogfood** → 适用时的 `R-MIG-8` 真实 profile 备份 / 校验 → V0.1 最终验收。**不得要求先对已知将被取代的 UI 做人工 dogfood**；人工 dogfood 是候选构建的**验收门**，不是实施的前置门。本条不降低「交付、安全与下一步」中的人工 dogfood 固定门槛（连续 7 天 / ≥20 篇）与三项人工数字要求。
...
- **[已确认·已实现@2026-09-09]** 未测试 ≠ 潜在不会：新目标下 unknown 仅表示“尚无显式反馈”，是否显示灰线由主动提示候选策略单独决定；未测词不得无条件成为灰线候选（见「查询、交互、主动提示与测评词包解耦」与「词汇状态与测试证据」）。
...
- **[已确认·已实现@2026-09-09]** 测评包是估计的唯一分母来源：查询词典内、固定测评包外的 wordKey 即使持有 `WordState` 或进入生词本，也不写入 `AssessmentEvidence`、不改变「是否已测过」「最久未测」、不进入估计分子或分母（与「查询、交互、主动提示与测评词包解耦」一致）。
...
- **[已确认·2026-09-10 裁决]** 追加两项 V0.1 明确不做：popup **Settings 页签 / 任何设置持久化**（DEC-2）与 popup **「本页 / This Page」页面词表**（DEC-4）。二者均为延后候选，完整理由与范围见「V0.1 呈现层与流程裁决」。
- **[已确认]** 高置信自动隐藏、Pool B、PAV/Beta 后验、概率画像、复杂三桶配额与受控回填仍不恢复。本轮「灰线只用于潜在不会候选」的高层方向已确认**且已实现**（2026-09-09 状态重标：原「但尚未实现」已被 fresh E2E 推翻——灰线现由频率下界阈值稀疏选出，实测 light/100 词 ≈ 4.55）；**OPEN_DECISIONS C 已决议验收方式并完成输入/算法对齐（dogfood 密度参考 + 不必要提示人工验收；候选输入＝仅频率 frq/bnc + 显式状态过滤，不含 Evidence/band；算法＝频率下界阈值，数字 dogfood 校准）**（见 Spec OPEN_DECISIONS C），实现须经后续 `/to-tickets` 阶段与用户“开始开发”授权；不得因本轮目标表述而默认恢复上述冻结项；系统预测不得改写用户明确状态（见「查询、交互、主动提示与测评词包解耦」）。
...
- **[已确认]** 2026-08-06 本轮仅修订 RULES 并新建唯一待审 Spec（`docs/specs/2026-08-06-V0.1-查询交互提示与测评词包解耦规格.md`，状态 DRAFT/待审）。**该 Spec 已于 2026-08-07 经第二道门收尾为 APPROVED / 已批准**（DOCUMENT 复审 `PASS`，审查基线 be4f289；用户最终批准 2026-08-07）；**OPEN_DECISIONS A–F 已由用户逐项决议，E/D/C 发现与对齐已完成并回写，D 已批准生产实施路线＝透明 span 包装 + 事件委托、C 输入/算法已对齐（bootstrap 公式不变：T₀ = S[⌊n/2⌋]，0-based，偶数取上侧元素，light iff rank > T₀）**；**批准 ≠ 实现（2026-09-09 状态重标）**——原句「完整解耦目标未实现……无灰线词可交互、候选稀疏、查询词典扩容等未实现」已被 fresh 验证推翻：**该解耦目标已实现**（无灰线词可交互、候选稀疏、查询词典已扩至 121,340 条，逐项见基线文档 §2–§3；Spec §15/§16 的「当前已实现＝否」表格已同步标注为过时）。不得据此拆 ticket、标记 ready-for-agent 或开始开发；拆票与开发须用户另行明确授权。**2026-09-09 后，主线的剩余差距不再来自本节，而是来自 UX 呈现层 delta（见 `docs/specs/2026-09-10-V0.1-UX-V1.2.1-集成规格.md` §7）。**
