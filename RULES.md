...
- **[已确认·尚未实现] Real reading feedback 持续校准（收口合同 3）**：单个 manual answer 不得让 boundary 大幅跳变；用与 calibration 一致的 transition-region 思路重新派生 boundary；explicit WordState 永远优先、聚合不得反写 WordState；无 rank 的 manual state 对该词本身有效但不参与 rank-boundary；数据不足保持上一个可信 boundary、从未形成则回退 T₀。**确定性施工合同见 Spec §9C**（合并视图 = calibration baseline + ranked manual WordState、同词 manual 优先、发布节流 M=3 后重跑 estimator、单条反馈不改变 boundary；M 为 DOGFOOD_TUNABLE_PARAMETER）。
