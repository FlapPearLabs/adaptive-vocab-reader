...
- **E/F（限定范围）**：仅个人本地 dogfood + load unpacked；公开再分发 UNKNOWN、fail-closed；包外词同键、不区分来源、不升级 schema；ECDICT 派生数据不入公开 Git（T-QD-1、T-NB-6）。
...
|---|---|---|---|---|
| 1 | [T-QD-1](T-QD-1-query-dictionary-asset-and-identity.md) | 查询词典资产与身份解耦（数据/构建层） | 无 | R-QUERY-1(数据)、R-QUERY-3、R-QUERY-5、R-ASSESS-1、R-COMPAT-4、R-HINT-3(合同) |
...
| 6 | [T-NB-6](T-NB-6-notebook-and-assessment-isolation.md) | 包外词状态、生词本与测评/估计隔离 | T-QD-1 | R-ASSESS-3、R-COMPAT-3、R-COMPAT-4 |
...
3. T-UNR-3/T-HINT-4/T-SEL-5/T-NB-6 在统一载体与合同上做行为扩展，互不重排架构；T-NB-6 与内容侧票文件边界不冲突，可并行（若用户日后允许并行任务）。
4. T-PERF-7 作为门禁在实现同批执行，防止「先合入再发现性能数量级问题」的返工。

## 三向一致性说明

- 全部 ticket 的 Requirement ID 均可回溯到 approved Spec §15 与 §9 AC / §10 seams；无 orphan requirement（§15 中标注「当前已实现/否」且不需要 ticket 的项：R-STATE-2、R-ASSESS-2、R-ASSESS-4(回归)、R-PRIVACY-1~4、R-PERF-2、R-COMPAT-1/2，均已实现或属文档/验证层面，见各票「是否可独立提交」与正文）。
- 无无 Spec 来源的 ticket requirement；无从旧 ticket/旧注释/历史 prototype 恢复的需求。
- 每个 ticket 均含真实 Chrome 验收路径 + 负断言 + 数据/许可边界 + 完成定义。
