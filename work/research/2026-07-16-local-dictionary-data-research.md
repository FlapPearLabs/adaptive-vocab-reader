...
**范围。** 这是 V0.1 的只读选型研究：Chrome 扩展需要可随扩展分发或由用户本机下载的英→简中词汇数据，并且要能说明覆盖、准确性、来源、许可证和更新方式。结论不构成法律意见；发布前仍应做许可证复核。

## 结论先行

不要把“本地词典”做成一份不可解释的大 CSV。推荐的是**可追溯的两层本地数据包**：
...
5. 下一步先做一个**离线构建 PoC**：选定单个 Kaikki 快照，构建 100 个可审计词条，产出 manifest、license notice、覆盖报告和 20 个多义词回归样例。通过后才决定核心包的体积预算和具体选词策略。
