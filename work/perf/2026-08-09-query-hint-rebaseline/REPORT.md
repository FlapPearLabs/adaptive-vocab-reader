...
本机本轮 SPA 导航中，真实 extension 请求的 query dictionary 为 `54.2 ms`，query forms 为 `54.1 ms`；从开始导航到首个内容脚本标注出现为 `860.0 ms`。后者是端到端 content-script ready 时间，含页面导航和扩展初始化，不能解释成纯 JavaScript 初始化时间。

| 场景 | 扫描墙钟 ms | 单批最大 ms | 文本节点 | 注释词数 | DOM 前→后（Δ） | scrollHeight 前→后（Δ） | 高度变化 px | CLS | 批次 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 长文 1 | 107.2 | 2.0 | 71 | 4159 | 79→4239（+4160） | 8948→8948（0） | 0 | 0 | 8 |
| 长文 2 | 112.2 | 2.1 | 71 | 4159 | 79→4239（+4160） | 8948→8948（0） | 0 | 0 | 8 |
| 长文 3 | 76.6 | 2.3 | 71 | 4159 | 79→4239（+4160） | 8948→8948（0） | 0 | 0 | 8 |
| SPA（初始 + append + route + characterData + CSS 交互） | 7.4 | 0.7 | 15 | 61 | 27→82（+55） | 600→611（+11） | 0 | 0 | 16 |
...
- `characterData` 将原始文本 `12345` 改为 query-eligible `serendipity` 后，真实 Chrome 记录 `batches=1`、`textNodesScanned=1`、`wordsAnnotated=1`、`totalScanMs=0.1`，并出现对应透明可查询 span。观测代码按已处理 batch 数决定是否发报告，因此即使时钟分辨率给出 `0 ms` 也会记录为 `0`，不会被 truthy 判断吞掉。
...
原型（2026-08-07）透明包装为 3,891 token、+3,888 DOM、4.0 ms、CLS 0；本次生产长文为 4,159 注释词、最终 DOM +4,160、76.6–112.2 ms、CLS 0。样本、词典规模和生产路径不同，不能把差异解释为回归或通过线。
