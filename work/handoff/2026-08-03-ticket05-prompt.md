...
- **仅限修正验收设施本身的问题**：`e2e-verify.cjs` 断言、隔离 fixture、必要构建加载入口与 `test:e2e` 脚本中属于验收设施自身缺陷的修正。**不得**借「验收升级」修改任何生产行为（`extension/src/**`）。

不可做：
- **不代修 T1~T4 产品缺陷**：综合回归若暴露 T1~T4 主责任缺陷，T5 只输出「不可进入 dogfood」+ 失败场景 + R-ID + 责任 Ticket + 复现证据，交由对应主责任范围的后续任务另行修复；修复合并 main 后再重新执行 T5 综合验收。T5 当前任务与分支不得直接修改生产代码
